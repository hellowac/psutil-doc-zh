...
发送信号(参阅 [信号模块手册])以抢先检查 PID 是否已被重用。在 UNIX 上，这与 `os.kill(pid, sig)` 相同。在 Windows 上仅支持 `SIGTERM` `、CTRL_C_EVENT` 和 `CTRL_BREAK_EVENT` 信号，并且 `SIGTERM` 被视为 [kill()] 的别名。另请参阅如何[终止进程树]和[终止子进程]。
